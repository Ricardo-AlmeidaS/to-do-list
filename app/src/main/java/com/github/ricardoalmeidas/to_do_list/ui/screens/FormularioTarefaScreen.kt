package com.github.ricardoalmeidas.to_do_list.ui.screens
...
import com.github.ricardoalmeidas.to_do_list.util.combinarDataHora
import com.github.ricardoalmeidas.to_do_list.util.extrairDataDoDatePicker
import com.github.ricardoalmeidas.to_do_list.util.paraMillisUtcDoDatePicker
...
fun FormularioTarefaScreen(
    ...
    FormularioTarefaContent(
        ...
        onSalvar = { titulo, descricao, dataHora ->
            if (tarefaId == 0) {
                viewModel.inserir(
                    Tarefa(titulo = titulo, descricao = descricao, dataHora = dataHora)
                )
            } else {
                tarefaExistente?.let {
                    viewModel.atualizar(
                        it.copy(titulo = titulo, descricao = descricao, dataHora = dataHora)
                    )
                }
            }
            onVoltar()
...
fun FormularioTarefaContent(
    ...
    if (mostrarSeletorData) {
        val estadoDatePicker = rememberDatePickerState(
            initialSelectedDateMillis = ano?.let { paraMillisUtcDoDatePicker(it, mes!!, dia!!) }
        )
        DatePickerDialog(
...
@Preview(showBackground = true, name = "Editar tarefa sem prazo")
@Composable
private fun FormularioTarefaContentEditarPreview() {
    FormularioTarefaContent(
        isEdicao = true,
        tituloInicial = "Estudar Room",
        descricaoInicial = "Entity, DAO e Database",
        dataHoraInicial = null,
        onSalvar = { _, _, _ -> },
        onVoltar = {}
    )
...
@Preview(showBackground = true, name = "Editar tarefa com prazo")
@Composable
private fun FormularioTarefaContentEditarComPrazoPreview() {
    ...
    FormularioTarefaContent(
        isEdicao = true,
        tituloInicial = "Entregar o trabalho",
        descricaoInicial = "Subir o projeto no GitHub",
